...
### Решение:
- Исправлены конфликты типов в `DocumentService.js` с добавлением проверок типов и преобразование
...
### Проблема:
- Передача избыточной информации в модель ИИ в виде JSON объектов вместо простых строк
- Лишнее сохранение документов перед анализом, приводящее к двойному сохранению
- Несогласованность типа ID документа (мог быть объектом вместо строки/null)
...
#### frontend/src/stores/documentStore.js
- **Добавлена проверка типа ID документа**: Вспомогательная функция `validateDocumentId` для проверки корректности типа ID
- **Улучшена логика определения PUT/POST запросов**: Строгая проверка типа ID при сохранении документов
- **Оптимизирована логика анализа документов**: Исключено лишнее сохранение перед анализом новых документов
- **Уточнена инициализация документов**: Явная установка `id: null` для новых документов

#### frontend/src/views/HomeView.vue
- **Изменена логика создания документов**: Убрано автоматическое сохранение при создании
...
3. **Улучшена стабильность**: Единый тип ID документа с проверками на корректность
4. **Улучшен пользовательский опыт**: Явный контроль над сохранением документов
5. **Упрощена архитектура**: Устранены избыточные операции сохранения и анализа
...
### Проблема:
- Неоптимальный интерфейс модального окна выбора надзорного органа:
  - Радиокнопки занимали много места и были неудобны для выбора
  - Надпись "Выберите надзорный орган" дублировалась в списке опций
  - Нелогичный порядок кнопок в диалоге (Отмена слева, Создать жалобу справа)

### Решение:
- Заменен интерфейс выбора органа с радиокнопок на выпадающий список
- Убрана дублирующая надпись из списка опций
- Добавлен placeholder "Выберите надзорный орган" в выпадающий список
- Улучшено визуальное оформление выпадающего списка с кастомной стрелкой
- Поменяны местами кнопки "Отмена" и "Создать жалобу" для лучшей логики UX
- Добавлена навигация к подробному просмотру жалобы после её успешной генерации

### Изменения в файлах:

#### frontend/src/views/DocumentsList.vue
- **Заменены радиокнопки на выпадающий список** в модальном окне выбора органа
- **Убрана дублирующая надпись** из списка опций выпадающего списка
- **Добавлен placeholder** "Выберите надзорный орган" в выпадающий список
- **Улучшено визуальное оформление** выпадающего списка с кастомной стрелкой
- **Поменяны местами кнопки** "Отмена" и "Создать жалобу" в диалоге
- **Добавлена навигация** к подробному просмотру жалобы после успешной генерации

### Результаты:
1. **Улучшен пользовательский интерфейс**: Более компактный и удобный выпадающий список вместо радиокнопок
2. **Устранено дублирование надписей**: Placeholder не отображается в списке опций
3. **Улучшена визуальная привлекательность**: Кастомная стрелка и улучшенные стили
4. **Логичный порядок кнопок**: Основная кнопка "Создать жалобу" теперь слева, "Отмена" справа
5. **Улучшен пользовательский опыт**: После генерации жалобы пользователь сразу переходит к её просмотру
...
### Проблема:
- Система классификации отделений ФССП по городам охватывала только ограниченное количество регионов
- Отсутствовали файлы с известными городами для большинства регионов Российской Федерации
- Низкая точность определения городов из адресов отделений из-за недостатка данных
...
- Названия населенных пунктов в каждом файле отсортированы по убыванию длины
- Учтены различные варианты написания названий населенных пунктов
- Создан скрипт для автоматической сортировки названий городов по длине
