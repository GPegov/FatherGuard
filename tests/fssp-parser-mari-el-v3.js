...
async function parseFSSPMariEl() {
  ...
  try {
    ...
    const departments = await page.evaluate(() => {
      ...
      // Работаем с первой таблицей (предполагаем, что она содержит нужные данные)
      ...
      // Начинаем с 5 строки (индекс 4), так как первые строки содержат контактные данные руководства
      for (let i = 4; i < rows.length; i++) {
        ...
        if (name && name.length > 5 && !name.includes('@') && !name.includes('mail')) {
          departments.push({
            name: name,
            address: address,
            phone: phone,
