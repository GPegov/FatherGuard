...
// Подготавливаем данные нарушений в правильном формате
const formattedViolations = realDocument.violations.map(violation => {
  if (typeof violation === 'object' && violation.lawName && violation.violationDescription) {
    return `${violation.lawName}, ${violation.violationDescription}`;
  } else if (typeof violation === 'object' && violation.article && violation.type) {
    return `Нарушение статьи ${violation.article}, тип нарушения: ${violation.type}`;
  } else {
...
const testData = {
  ...
  currentDocument: {
    ...
    documentDate: realDocument.documentDate || '2025-08-30',
    senderAgency: realDocument.senderAgency || 'Отдел судебных приставов',
  ...
  relatedDocuments: dbData.documents.slice(1, 3).map(doc => ({
    id: doc.id,
...
async function testRealFSSPData() {
  try {
    ...
    if (complaint.templatedContent) {
      ...
      const outputPath = path.join(__dirname, 'test-real-fssp-output-improved.html');
    ...
    if (complaint.content) {
      ...
      const textPath = path.join(__dirname, 'test-real-fssp-complaint-improved.txt');
